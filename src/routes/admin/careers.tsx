...
function AdminCareers() {
  ...
  const [json, setJson] = useState(JSON.stringify(data.careers, null, 2))
  const [msg, setMsg] = useState('')

  return (
    <div className="font-sans text-text">
      <AdminNavbar active="Careers in Terionix" />
      <main className="pt-32 pb-24 px-4">
        <div className="max-w-3xl mx-auto">
          <h1 className="font-title text-3xl font-bold mb-2">Careers</h1>
          ...
          <button onClick={async () => {
            try { await updateSection({ data: { section: 'careers', content: JSON.parse(json) } }); setMsg('Saved!') }
